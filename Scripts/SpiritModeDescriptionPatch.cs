...
// 瞄准某个单位时则按瞄准目标阵营返回对应分支描述；
// 基酒 hover tip 中的鸡尾酒预览牌则跟随来源基酒右键切换的 SpiritMode 分支
// （只影响本地显示，不改变实际效果）。
...
public static class SpiritModeDescriptionPatch
{
    ...
    public static void Postfix(CardModel __instance, ref LocString __result)
    {
        ...
        if (__instance is ISpiritModeCard spiritCard)
        {
            ...
            if (aimBranch.HasValue)
            {
                __result = spiritCard.GetSpiritDescriptionFor(aimBranch.Value);
                return;
            }

            // 基酒 hover tip 中的鸡尾酒预览：跟随来源基酒右键切换的 SpiritMode 分支。
            // 预览牌是从卡池实例克隆出的独立实例，不影响真正的鸡尾酒牌。
            if (SpiritModeHoverPreview.TryGetBranch(__instance, out SpiritTargetBranch previewBranch))
            {
                __result = spiritCard.GetSpiritDescriptionFor(previewBranch);
                return;
            }
