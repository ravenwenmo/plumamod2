...
// 玛格丽特：0费生成技能牌，鸡尾酒。目标为任意单位（敌人/自己/友方），实际效果由所选目标的阵营决定；右键仅在本机循环切换提示与光效，不影响实际效果。
// 效果：对敌人造成10点伤害并施加1层虚弱；对自己/友方造成1点穿透伤害后获得1层收割者。升级后伤害+3并获得保留。
[RegisterCard(typeof(TokenCardPool))]
public class Margarita : ModCardTemplate, IModRightClickableCard, ISpiritModeCard, ICocktailCard, IBaseSpiritRelatedCard
{
    ...
    {
        SpiritMode.Self  => new LocString("cards", "PLUMA_CARD_MARGARITA_SELF_DESC"),
        SpiritMode.Enemy => new LocString("cards", "PLUMA_CARD_MARGARITA_ENEMY_DESC"),
        SpiritMode.Ally  => new LocString("cards", "PLUMA_CARD_MARGARITA_ALLY_DESC"),
        _ => new LocString("cards", "PLUMA_CARD_MARGARITA_SELF_DESC")
    };
    ...
    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        ...
        switch (SpiritTargeting.Resolve(cardPlay.Target, base.Owner.Creature))
        {
            ...
            case SpiritTargetBranch.Ally:
                ...
                // 友方玩家效果：获得收割者
                await PowerCmd.Apply<ReaperHealingPower>(
                    choiceContext,
                    cardPlay.Target,
                    DynamicVars["ReaperHealingAmount"].BaseValue,
                    base.Owner.Creature,
                    this
                );
                break;
