...
// 龙舌兰：0费生成技能牌，基酒。目标为任意单位（敌人/自己/友方），实际效果由所选目标的阵营决定；右键仅在本机循环切换提示与光效，不影响实际效果。
// 效果：对敌人造成3点伤害并施加1层虚弱；对自己/友方造成1点穿透伤害后施加3层再生。
[RegisterCard(typeof(TokenCardPool))]
public class Tequila : ModCardTemplate, IModRightClickableCard, IBaseSpiritCard, ISpiritModeCard
{
    ...
    {
        SpiritMode.Self  => new LocString("cards", "PLUMA_CARD_TEQUILA_SELF_DESC"),
        SpiritMode.Enemy => new LocString("cards", "PLUMA_CARD_TEQUILA_ENEMY_DESC"),
        SpiritMode.Ally  => new LocString("cards", "PLUMA_CARD_TEQUILA_ALLY_DESC"),
        _ => new LocString("cards", "PLUMA_CARD_TEQUILA_SELF_DESC")
    };
    ...
    public Tequila() : base(energyCost, CardType.Skill, rarity, PlumaTargetTypes.AnyUnit, shouldShowInCardLibrary)
    {
    }
    ...
    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        switch (SpiritTargeting.Resolve(cardPlay.Target, base.Owner.Creature))
        {
            case SpiritTargetBranch.Self:
                ...
                // 施加 3 层再生
                await PowerCmd.Apply<RegenPower>(choiceContext, base.Owner.Creature, 4,
            ...
            case SpiritTargetBranch.Ally:
                ...
                await CreatureCmd.Damage(choiceContext, cardPlay.Target!, 1,
                    ValueProp.Unblockable | ValueProp.Unpowered, null, null);
                // 施加 3 层再生
                await PowerCmd.Apply<RegenPower>(choiceContext, cardPlay.Target, 4,
                    base.Owner.Creature, this);
