...
public class HealTequilaOption : RestSiteOption
{
    public override string OptionId => "Pluma_HealTequila";
    ...
    public HealTequilaOption(Player owner)
		: base(owner)
	{
	}
    ...
    public static decimal GetHealAmount(Player player)
	{
		...
		GD.Print($"Final heal amount for Tequila: {healAmount}");
		return healAmount;
	}

    public static decimal GetBaseHealAmount(Player player)
	{
        int maxHp = TequilaState.GetMaxHp(player);
        GD.Print($"Calculating base heal amount for Tequila: Max HP = {maxHp}, Base Heal = {maxHp * 0.3m * 2}");
		return maxHp * 0.3m * 2;
	}
    ...
    {
        TequilaState.Heal(player, (int)GetHealAmount(player));
    }
}